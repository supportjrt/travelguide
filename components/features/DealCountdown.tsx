...
export default function DealCountdown({ onClaim }: DealCountdownProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white rounded-2xl p-6 md:p-8 relative overflow-hidden shadow-2xl border border-gray-700/50">
      ...
      <div className="relative z-10 flex flex-col items-start w-full">
        ...
        <div className="w-full border-t border-gray-700/50 pt-6">
          ...
          <div className="flex items-center justify-between w-full max-w-sm">
            ...
            <span className="text-lg md:text-2xl font-bold text-gray-600 -mt-4">:</span>
...
function TimeBox({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="bg-gray-800 rounded-lg w-14 h-14 md:w-16 md:h-16 flex items-center justify-center mb-2 shadow-inner border border-gray-700">
        <span className="text-xl md:text-3xl font-mono font-bold text-white">
          {value < 10 ? `0${value}` : value}
        </span>
      </div>
      <span className="text-[9px] md:text-[10px] font-bold text-gray-500 tracking-wider">{label}</span>
    </div>
  );
}
